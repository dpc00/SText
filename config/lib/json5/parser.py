...
import re as re  # PATCH(vendor): was `import regex as re`; stdlib re suffices for ASCII ST config keys
...
class JSONParser(Parser):  # type: ignore[misc]
    ...
    def identifier(self, p: T_IdentifierProduction) -> Identifier:
        ...
        pattern = r'[\w_\$]([\w_\d\$\u200C\u200D])*'  # PATCH(vendor): dropped \p{Pc}\p{Mn}\p{Mc} (regex-lib only; ASCII keys never use them)
