...
command defined in one of these modules but not imported into
...
(ai_terminal.py and its registration/keymap guards now live in the GhostShell
repo -- SText's PluginLoader no longer imports it.)

Both files are parsed with ``ast`` rather than imported, since importing some
of these modules outside Sublime is not possible.
"""
